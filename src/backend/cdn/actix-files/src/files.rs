...
impl Files {
    ...
    ///
    /// # Argument Order
    /// The first argument (`mount_path`) is the root URL at which the static
    /// files are served. For example, `/assets` will serve files at
    /// `example.com/assets/...`.
    ///
    /// The second argument (`serve_from`) is the location on disk at which
    /// files are loaded. This can be a relative path. For example, `./`
    /// would serve files from the current working directory.
    ///
    /// # Implementation Notes
    /// If the mount path is set as the root path `/`, services registered after
    /// this one will be inaccessible. Register more specific handlers and
    /// services first.
    ///
    /// `Files` utilizes the existing Tokio thread-pool for blocking filesystem
    /// operations. The number of running threads is adjusted over time as
    /// needed, up to a maximum of 512 times the number of server
    /// [workers](actix_web::HttpServer::workers), by default.
    ...
    ///
    /// When used with [`Files::index_file()`], files listing is shown as a
    /// fallback when the index file is not found.
    ...
    /// You can safely join this path with the `serve_from` path to get the real
    /// path. However, the real path may not exist since the filter is
    /// called before checking path existence.
    ///
    /// When a path doesn't pass the filter, [`Files::default_handler`] is
    /// called if set, otherwise, `404 Not Found` is returned.
    ///
    /// # Examples
    /// ```
    /// use {
    ...
    ///
    /// Use this to allow multiple chained file services that respond to
    /// strictly different properties of a request. Due to the way routing
    /// works, if a guard check returns true and the request starts being
    /// handled by the file service, it will not be able to back-out and try
    /// the next service, you will simply get a 404 (or 405) error response.
    ///
    /// To allow `POST` requests to retrieve files, see
    /// [`Files::method_guard()`].
    ///
    /// # Examples
    /// ```
    ...
    /// };
    ///
    /// App::new().service(Files::new("/", "/my/site/files").guard(Header("Host", "example.com")));
    /// ```
    ...
    ///
    /// Note that this guard has no effect on routing; it's main use is to guard
    /// on the request's method just before serving the file, only allowing
    /// `GET` and `HEAD` requests by default. See [`Files::guard`] for
    /// routing guards.
    ...
    ///     .default_handler(fn_service(|req: ServiceRequest| {
    ///         async {
    ...
    /// Enables serving hidden files and directories, allowing a leading dots in
    /// url fragments.
