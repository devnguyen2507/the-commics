...
impl NamedFile {
    ...
    ///
    /// The given `path` need not exist and is only used to determine the
    /// `ContentType` and `ContentDisposition` headers.
    ///
    /// # Examples
    /// ```ignore
    /// use std::{
    ...
    ///
    /// When the `experimental-io-uring` crate feature is enabled, this will be
    /// async. Otherwise, it will behave just like `open`.
    ///
    /// # Examples
    /// ```
    /// use actix_files::NamedFile;
    ...
    ///
    /// Returns `None` only on unsupported platforms; see
    /// [`std::fs::Metadata::modified()`]. Therefore, it is usually safe to
    /// unwrap this.
    #[inline]
    ...
    /// Returns the `Content-Type` header that will be used when serving this
    /// file.
    #[inline]
    ...
    /// Returns the `Content-Disposition` that will be used when serving this
    /// file.
    #[inline]
    ...
    ///
    /// A return value of `None` indicates that the content is not already using
    /// a compressed representation and may be subject to compression
    /// downstream.
    #[inline]
    ...
    /// Set response Cache-Control Max Age.
    #[inline]
    pub fn set_cache_ttl(mut self, ttl: usize) -> Self {
        self.cache_ttl = ttl;
        self
    }

    /// Sets the `Content-Type` header that will be used when serving this file.
    /// By default the `Content-Type` is inferred from the filename
    /// extension.
    #[inline]
    ...
    /// Set the Content-Disposition for serving this file. This allows changing
    /// the `inline/attachment` disposition as well as the filename sent to
    /// the peer.
    ///
    /// By default the disposition is `inline` for `text/*`, `image/*`,
    /// `video/*` and `application/{javascript, json, wasm}` mime types, and
    /// `attachment` otherwise, and the filename is taken from the path
    /// provided in the `open` method after converting it to UTF-8
    ...
    ///
    /// This prevents the `Compress` middleware from modifying the file contents
    /// and signals to browsers/clients how to decode it. For example, if
    /// serving a compressed HTML file (e.g., `index.html.gz`) then use
    ...
    pub fn into_response(self, req: &HttpRequest) -> HttpResponse<BoxBody> {
        ...
        if self.cache_ttl > 0 {
            res.insert_header((
                ...
                format!("public, max-age={}", self.cache_ttl),
            ));
        } else {
            res.insert_header((
                header::CACHE_CONTROL,
