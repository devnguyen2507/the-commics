...
//! };
//!
//! let app = App::new().service(Files::new("/static", ".").prefer_utf8(true));
//! ```
...
/// (case-insensitive). If `ext` is empty or no associated type for the
/// extension was found, returns the type `application/octet-stream`.
#[inline]
...
mod tests {
    ...
    async fn test_static_files_with_special_characters() {
        // Create the file we want to test against ad-hoc. We can't check it in as
        // otherwise Windows can't even checkout this repository.
    ...
    async fn test_default_handler_file_missing() {
        let st = Files::new("/", ".")
            .default_handler(|req: ServiceRequest| {
                async { Ok(req.into_response(HttpResponse::Ok().body("default content"))) }
            })
            .new_service(())
            .await
            .unwrap();
