...
- **Các tệp đã sửa đổi:**
  - **`src/ui/src/pages/index.astro`**: 
    - Cập nhật `pageTitle`: `Truyện Hentai, Đọc Truyện Tranh 18+ Manhwa Không Che - FanManga`.
    - Cập nhật thẻ `meta description` kêu gọi hành động.
    - Chỉnh sửa thẻ `<h1/>` nội dung SEO Body chứa các từ khóa lõi (`đọc truyện tranh 18+`, `hentai`, `manhwa vietsub`).
