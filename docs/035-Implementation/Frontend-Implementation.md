...
### 2.1 Các Route Chính
- `/`: Homepage - Chứa HeroSlider và lưới truyện Mới Cập Nhật. Dùng `<ComicCard>`. Lấy cố định theo cấu hình Env.
- `/truyen-moi`: Danh sách truyện mới cập nhật, hỗ trợ `?page=X`.
- `/truyen-hot`: Danh sách truyện xem nhiều (`sort: MOST_VIEWED`), hỗ trợ `?page=X`.
- `/truyen-ngan`: Danh sách truyện loại Truyện ngắn, hỗ trợ `?page=X`.
- `/tim-kiem?q=keyword`: Trang kết quả tìm kiếm. Đồng nhất lưới truyện như trang chủ, hỗ trợ `?page=X`. Lọc theo biến query `searchQuery`.
- `/truyen/{slug}`: Chi tiết truyện. Hiển thị Chapter List, Nội dung Description.
- `/truyen/{slug}/chap-{chapterNum}`: Trang đọc truyện (Reader). Chứa logic cuộn ảnh mượt mà và pre-fetch. URL được chuẩn hóa dạng `chap-N` và slugify (VD: `chap-ch-1`).
...
- Phân trang & Điều hướng SEO: Component tĩnh `<Pagination />` truyền vào `currentPage` và `totalPages` kèm `noindex` tự động khi > 1 để chống duplicate search results content từ Google.
...
| `PUBLIC_CDN_URL` | `getRuntimeEnv` (runtime) | Image URLs (nối tiền tố CDN) |
| `PUBLIC_HOME_COMICS_LIMIT` | `import.meta.env` (build) | Giới hạn truyện / related mục Home (24) |
...
| `sitemap-comics.xml.ts` | `/truyen/[slug]` | `getComics({ first: 5000 })` |
| `sitemap-chapters.xml.ts` | `/truyen/[slug]/chap-[chapterNum]` | comics.chapters[] |
