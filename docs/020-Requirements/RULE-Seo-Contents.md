...
### 2.12 — UTILITY & LISTING PAGES (NGOẠI LỆ SEO)

**1. Nhóm Utility Pages** (`/lien-he/`, `/dieu-khoan/`, `/chinh-sach/`, `/cau-hoi-thuong-gap/`)
| Rule | Chi tiết |
|------|----------|
| ✅ BẮT BUỘC | Title tag có Brand. |
| ✅ BẮT BUỘC | Schema phù hợp với trang (ContactPage, AboutPage, FAQPage). |
| ❌ CẤM | Không nhồi nhét keyword "Manga/Manhwa" vào nội dung trang. |
| ❌ CẤM | Không ép buộc phải có 3 - 5 internal linking chứa anchor text manga trong body content. (Chỉ cần footer là đủ tính kết nối). |
| 🔄 MỤC ĐÍCH | Tối ưu trải nghiệm E-E-A-T (Chuyên môn - Trải nghiệm - Thẩm quyền - Độ tin cậy) thay vì ranking top từ khóa. |

**2. Nhóm Listing Pages** (`/truyen-hot/`, `/truyen-moi/`, `/tim-kiem/`, `/the-loai/`)
| Rule | Chi tiết |
|------|----------|
| ✅ BẮT BUỘC | Title tag phải có ngữ cảnh mang tính tổng hợp / cập nhật. Ví dụ: "Top", "Hot", "Mới Cập Nhật Hôm Nay".  |
| ✅ BẮT BUỘC | H1 phải khớp nội dung, khác Title tag và phải chứa cụm từ khóa. Ví dụ H1: "Top Truyện Tranh Hot Nhất". |
| ✅ BẮT BUỘC | (Khuyến khích) Có một SEO Content Block (200-300 chữ) ở cuối trang để tránh Lỗi Thin Content. **Heading của SEO Block bắt buộc dùng thẻ H2**, tuyệt đối không dùng H1 để tránh vi phạm Rule 1.4 (Multiple H1s). |
| 🔄 MỤC ĐÍCH | Rank các từ khóa mang tính chất Navigation (Điều hướng) và Transactional (Tìm để đọc ngay). |

---

### 2.13 — YÊU CẦU OUTPUT KHI ĐÁNH GIÁ SEO CONTENT (SEO AUDIT)

Khi tiếp nhận yêu cầu review/đánh giá SEO content của một trang cụ thể, kết quả đầu ra (output) BẮT BUỘC phải tuân thủ format sau để đảm bảo tính khách quan và dễ dàng triển khai:

**1. So sánh toàn diện (Comprehensive Comparison):**
- Phải đối chiếu nội dung trang hiện tại với **TOÀN BỘ các tiêu chí tương ứng** trong Phần 2 (ví dụ: Trang Truyện thì đối chiếu mục 2.5, Trang Thể Loại thì đối chiếu mục 2.4).
- Ghi nhận rõ Output/Tình trạng hiện tại (Current Status) của trang đối với từng tiêu chí (Pass / Fail / Cần tối ưu thêm).

**2. Đánh giá mức độ nghiêm trọng (Severity & Priority):**
Với mỗi lỗi hoặc điểm chưa đạt, phải gắn nhãn mức độ nghiêm trọng để team phân bổ ưu tiên xử lý:
- 🔴 **Critical (Nghiêm trọng):** Lỗi ảnh hưởng trực tiếp đến ranking hoặc vi phạm technical (VD: Thiếu H1, Multiple H1s, sai Canonical, Index Bloat). Yêu cầu fix ngay.
- 🟡 **High (Cao):** Lỗi ảnh hưởng lớn đến CTR hoặc mức độ hiểu của bot (VD: Sai Title/Meta, thiếu Alt text quan trọng, keyword stuffing).
- 🔵 **Medium/Low (Trung bình/Thấp):** Các cơ hội tối ưu thêm (VD: Bổ sung FAQ, thêm LSI keyword, tối ưu text content blocks).

**3. Đề xuất giải pháp (Proposed Changes & Tradeoffs):**
- Đưa ra chính xác hành động cần thay đổi (ví dụ: Before vs After content/code).
- **Nếu có nhiều hơn 1 đề xuất (Multiple Options):**
  - Liệt kê TẤT CẢ các đề xuất.
  - Đánh giá tính **Khả thi (Feasibility)** về nỗ lực Dev/Content.
  - Phân tích rạch ròi **Trade-off (Được/Mất)** của từng phương án (Ví dụ: Chèn text quá dài trên Chapter page tốt cho text signals nhưng ảnh hưởng UX trải nghiệm đọc → đề xuất UI ẩn bớt text).
  - Chọn ra phương án Recommend tốt nhất để chốt hạ.
