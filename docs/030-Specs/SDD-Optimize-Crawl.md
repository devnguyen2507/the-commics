...
- **Sitemaps:** Triển khai thẻ `lastmod` bắt buộc bằng trường `publishedAt` cho cả Truyện (`Comic`) và Chương (`Chapter`). Tránh sử dụng `updatedAt` để đồng nhất theo yêu cầu. Toàn bộ dữ liệu cũ sẽ được migration về mốc thời gian cố định.
...
*   **Sử dụng `publishedAt` làm `lastmod`:** 
    ...
    *   **Migration:** Tất cả các bản ghi hiện có sẽ được gán giá trị mặc định: `2026-03-07T07:56:40.915Z`.
